class Property < ApplicationRecord
  ...
  has_many :reserved_user, through: :reservations, source: :user, dependent: :destroy

  has_many :payments, through: :reservations, dependent: :destroy

  has_rich_text :description

  def self.with_reservation_overlap(checkin_data, checkout_date)
    where.not(id: Reservation.overlapping_reservation(checkin_data, checkout_date).pluck(:property_id))
  end
  ...
  def available_date
    ...
    current_reservation = reservations.current_reservation.first
    ...
    if current_reservation.nil? && next_reservation.nil?
      ...
      Date.tomorrow.strftime("%e %b")..next_reservation.checkin_data.strftime("%e %b")
    elsif next_reservation.nil?
      current_reservation.checkout_date.strftime("%e %b")..(Date.tomorrow + 30.days).strftime("%e %b")
    else
      current_reservation.checkout_date.strftime("%e %b")..next_reservation.checkin_data.strftime("%e %b")
    end
  end
end
